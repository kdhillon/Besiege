...
public class BackgroundColor implements Drawable {
    ...
    private void initialize(String filename) {
        ...
        if (x == null || y == null) setPosition();    // x = 0.0f; y = 0.0f;
        if (width == null || height == null || width < 0.0f || height < 0.0f)
            setSize();        // width = 0.0f; height = 0.0f;
        if (color == null) setColor(255, 255, 255, 255);
        if (sprite == null) {
            try {
                setSprite();
            } catch (Exception e) {
    ...
    private void updateSprite(float x, float y, float width, float height) {
        if (fillParent) {
            ...
            if (width != textureRegion.getRegionWidth() || height != textureRegion.getRegionHeight()) {
