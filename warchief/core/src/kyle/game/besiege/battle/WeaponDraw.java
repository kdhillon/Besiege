...
public class WeaponDraw extends Group { // can have arrows.
	...
	private Animation<TextureRegion> horseWalk;
...
//		this.toDraw = new TextureRegion(new Texture(Gdx.files.internal("weapons/axe")));
		...
		if (unit.horse != null) {
			...
			horseWalk = new Animation<TextureRegion>((float)Math.random()*.05f+.25f, textureArray2[0]);
			horseWalk.setPlayMode(Animation.PlayMode.LOOP);
		}
		if (unit.shield != null) {
...
//			weaponRangedTexture = Assets.weapons.findRegion("Longbow");
	...
	public void drawShadow(Batch batch, float x, float y, float width, float height) {
		width *= 0.8f;
		height *= 0.8f;
