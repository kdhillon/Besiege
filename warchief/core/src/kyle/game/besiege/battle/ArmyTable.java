...
public class ArmyTable extends Table {
    ...
    private void initializeCultureOptions() {
        StrictArray<SelectOption<CultureType>> cultureOptions = new StrictArray<SelectOption<CultureType>>();
    ...
    private void initializePartyTypeOptions() {
        StrictArray<SelectOption<PartyType.Type>> partyTypeOptions = new StrictArray<SelectOption<PartyType.Type>>();

        StrictArray<PartyType.Type> toAddLater = new StrictArray<PartyType.Type>();
        ...
        partyTypeOptions.add(new SelectOption<PartyType.Type>(toAddLater.toArray(), "Random"));

        for (PartyType.Type type : toAddLater) {
            if (type == null) continue;
            partyTypeOptions.add(new SelectOption<PartyType.Type>(type, formatForList(type.toString())));
        }

        // Add stuff to the table
        partyType = new QuickBattleTable.TextWithDropdown<SelectOption<PartyType.Type>>("Party Type", partyTypeOptions.toArray());
