...
public class QuickBattleTable extends Table {
    ...
    public static class TextWithDropdown<T> extends Table {
        ...
        public TextWithDropdown(String labelText, T[] items) {
            ...
            this.options = new SelectBox(new SelectBox.SelectBoxStyle(fontForEverything, Color.WHITE, Assets.ninepatchBackgroundDarkGray, spStyle, listStyle));
        ...
        public T getSelected() {
            return items[options.getSelectedIndex()];
        }
    }
...
//        mapOptions.debug();
    ...
    private void initializeWeatherTypes() {
        ...
        for (BattleOptions.WeatherEffect weatherEffect : BattleOptions.WeatherEffect.values()) {
            ...
            options.add(new SelectOption<BattleOptions.WeatherEffect>(weatherEffect, name));
        }

        weatherEffect = new TextWithDropdown<SelectOption<BattleOptions.WeatherEffect>>("Weather", options.toArray());
    ...
    private void initializeTimeOfDay() {
        ...
        for (BattleOptions.TimeOfDay timeOfDay : BattleOptions.TimeOfDay.values()) {
            options.add(new SelectOption<BattleOptions.TimeOfDay>(timeOfDay, formatForList(timeOfDay.toString())));
        }

        timeOfDay = new TextWithDropdown<SelectOption<BattleOptions.TimeOfDay>>("Time of Day", options.toArray());
    ...
    private void initializeLocationTypes() {
        ...
        options.add(new SelectOption<Location.LocationType>(Location.LocationType.values(), "Random"));
        ...
        for (Location.LocationType type : Location.LocationType.values()) {
            ...
            options.add(new SelectOption<Location.LocationType>(type, formatForList(type.toString())));
        }

        locationType = new TextWithDropdown<SelectOption<Location.LocationType>>("Siege Type", options.toArray());
        mapOptions.add(locationType).expandX().fillX();
        mapOptions.row();
    }

    private void initializeMapTypes() {
        StrictArray<SelectOption> options = new StrictArray<SelectOption>();

        // TODO add some other options.
        options.add(new SelectOption<BattleMap.MapType>( BattleMap.MapType.values(), "Random"));

        for (BattleMap.MapType type : BattleMap.MapType.values()) {
            options.add(new SelectOption<BattleMap.MapType>(type, formatForList(type.toString())));
        }
    ...
    private void initializeDefending() {
        ...
        // TODO add some other options.
        options.add(new SelectOption<Boolean>(true, "Defending"));
        options.add(new SelectOption<Boolean>(false, "Attacking"));
