...
public class BattleMap extends Group {
	...
	private StrictArray<GroundType> groundTypes = new StrictArray<GroundType>();
...
//	}
	...
	public void drawShadow(Batch batch, TextureRegion texture, float x, float y, float width, float height) {
...
//					System.out.println("encountered duplicate point in battlemap. good");
	...
	private StrictArray<BPoint> getRadiusPoints(Unit unit, int range) {
		StrictArray<BPoint> points = new StrictArray<BPoint>();
	...
	private void drawRadius(Unit drawRange, Batch batch, boolean diminishing, Color color, int range, boolean quarter) {
			Color c = batch.getColor();
	...
	private boolean drawRange(Unit drawRange, Batch batch) {
	...
	void drawLOS(Unit drawLos, Batch batch) {
		drawRadius(drawLos, batch, false, LOS_COLOR, drawLos.getLineOfSight(), false);
	}

	void drawHideRadius(Unit unit, Batch batch) {
