...
//			size_factor = .8f * size_factor;
		...
		if (shouldDrawCrest()) {
			...
			float yPos = -16*zoom;
			batch.draw(Assets.white, -(int) (4.3*toDraw.length())*zoom - zoom * fontWidthScale / 10, yPos - fontHeight*zoom * 0.9f,  (9*toDraw.length() + 12)*zoom, zoom*fontHeight);
			batch.setColor(c);
...
//			font.draw(batch, toDraw, -(int) (4.3*toDraw.length())*zoom * bgScale, -8*zoom * bgScale);
	...
	//	public void drawInfo(Batch batch, float parentAlpha) {
