...
//		}
        ...
        UnitDraw.drawUnit(this, batch, walkArmor, walkSkin, getGeneralArmorColor(), getGeneralSkinColor(), animationTime, getGeneral().getEquipment(), getAlphaForAmbush());
...
//		this.setKingdomRotation(kingdomRotation);
	...
	public void drawCrest(Batch batch) {
...
//
//    protected void drawAnimationTint(Batch batch, Animation animation, float stateTime, boolean loop, Color tint) {
...
//    }
	...
	public float getAlphaForAmbush() {
		if (ambushStarted) {
			float ambushColor = 0.5f;
			if (!isInAmbush()) {
				return Math.min(1, ambushColor + 0.1f + (1 - timeSinceAmbushSet / TIME_TO_SET_AMBUSH) * (1 - ambushColor));
			} else {
				return ambushColor;
			}
		}
